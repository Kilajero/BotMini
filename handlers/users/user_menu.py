...
async def open_category_for_create_position(call: CallbackQuery, state: FSMContext):
    ...
    send_msg = f"<b>🎁 Покупка товара:</b>\n" \
               ...
               f"<b>💵 Стоимость:</b> <code>{get_position[3]} USD</code>\n" \
               f"<b>📦 Количество:</b> <code>{len(get_items)}шт</code>\n" \
               f"<b>📜 Описание:</b>\n" \
               f"{get_position[4]}\n"
    if len(get_position[5]) >= 5:
        await call.message.delete()
        await call.message.answer(send_msg,
    ...
    if len(get_items) >= 1:
        if int(get_user[4]) >= int(get_position[3]):
            ...
            await call.message.answer(f"📦 <b>Введите количество товаров для покупки</b>\n"
                                      ...
                                      f"💵 Стоимость товара: <code>{get_position[3]} USD</code>\n"
                                      f"💳 Ваш баланс: <code>{get_user[4]} USD</code>\n",
            ...
            if 1 <= get_count <= len(get_items):
                if int(get_user[4]) >= amount_pay:
                    ...
                    await message.answer(f"<b>🎁 Вы действительно хотите купить товар(ы)?</b>\n"
                                         ...
                                         f"💰 Сумма к покупке: <code>{amount_pay} USD</code>",
    ...
    if 1 <= int(get_count) <= len(get_items):
        if int(get_user[4]) >= amount_pay:
            ...
            for item in save_items:
                if 'photo:' in item:
                    await bot.send_photo(call.from_user.id, item.replace('photo:', ''))
                else:
                    new_arr.append(item)
            save_items = new_arr
            ...
            for item in save_items:
                if 'document:' in item:
                    await bot.send_document(call.from_user.id, item.replace('document:', ''))
                else:
                    new_arr.append(item)
            save_items = new_arr
            ...
            await call.message.answer(f"<b>🎁 Вы успешно купили товар(ы) ✅</b>\n"
                                      ...
                                      f"💵 Сумма покупки: <code>{amount_pay} USD</code>\n"
