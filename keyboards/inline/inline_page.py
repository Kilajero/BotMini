...
def position_open_edit_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            ...
            keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]} | {get_positions[a][3]} USD | {len(get_items)}шт",
...
def buy_item_item_position_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            get_items = get_itemsx("*", position_id=get_positions[a][1])
            keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]} | {get_positions[a][3]} USD",
